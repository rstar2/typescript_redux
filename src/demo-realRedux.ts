...
class MessageActions {
  static createAddMessage(message: string): AddMessageAction {
    return {
      type: 'ADD_MESSAGE',
      message: message
    };
  }
  static createDeleteMessage(index: number): DeleteMessageAction {
    return {
      type: 'DELETE_MESSAGE',
      index: index
    };
...
store.dispatch(
  MessageActions.createAddMessage('Would you say the fringe was made of silk?'));

store.dispatch(
  MessageActions.createAddMessage('Wouldnt have no other kind but silk'));

store.dispatch(
  MessageActions.createAddMessage('Has it really got a team of snow white horses?'));
